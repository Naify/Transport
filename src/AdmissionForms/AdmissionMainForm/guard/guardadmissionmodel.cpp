...
GuardAdmissionModel::GuardAdmissionModel(int vcId, QObject *parent):
...
GuardAdmissionModel::~GuardAdmissionModel()
{
}

void GuardAdmissionModel::init()
{
    load();
}

void GuardAdmissionModel::load()
{
    ...
    ODS::IObjectCursor cursor = ODS::OdsInterface::self()->iobjectManager().getIObjectCursor("Допуска КП.Допуска караульные",filter);
...
void GuardAdmissionModel::makeRow(const ODS::IObject &obj)
{
...
void GuardAdmissionModel::addEmptyRow()
{
    QList<QStandardItem*> list;
...
void GuardAdmissionModel::choosePerson()
{
...
void GuardAdmissionModel::remove(const QModelIndex &index)
{
...
bool GuardAdmissionModel::save()
{
    ...
    for (int i = 0; i< rowCount(); i++){
        ...
        if (id > 0)
            ...
            obj = ODS::OdsInterface::self()->iobjectManager().createIObject("Допуска КП.Допуска караульные");
...
bool GuardAdmissionModel::saveItem(QStandardItem *item, ODS::IObject &obj)
{
...
void GuardAdmissionModel::changeVc(int vcId)
{
    removeRows(0, rowCount());
    currentVcid = vcId;

    ODS::IObjectCursor cursor = ODS::OdsInterface::self()->iobjectManager().getIObjectCursor("Допуска КП.Допуска ЗНЖДЭ",
