...
KranAdmissionModel::KranAdmissionModel(int vcId, QObject *parent):
...
KranAdmissionModel::~KranAdmissionModel()
{
}

void KranAdmissionModel::init()
{
    load();
}

void KranAdmissionModel::load()
{
    ...
    ODS::IObjectCursor cursor = ODS::OdsInterface::self()->iobjectManager().getIObjectCursor("Допуска КП.Допуска крановщики",filter);
...
void KranAdmissionModel::makeRow(const ODS::IObject &obj)
{
...
void KranAdmissionModel::addEmptyRow()
{
    QList<QStandardItem*> list;
...
void KranAdmissionModel::choosePerson()
{
...
void KranAdmissionModel::remove(const QModelIndex &index)
{
...
bool KranAdmissionModel::save()
{
    ...
    for (int i = 0; i< rowCount(); i++){
        ...
        if (id > 0)
            ...
            obj = ODS::OdsInterface::self()->iobjectManager().createIObject("Допуска КП.Допуска крановщики");
...
bool KranAdmissionModel::saveItem(QStandardItem *item, ODS::IObject & obj)
{
...
void KranAdmissionModel::changeVc(int vcId)
{
    removeRows(0, rowCount());
    currentVcid = vcId;

    ODS::IObjectCursor cursor = ODS::OdsInterface::self()->iobjectManager().getIObjectCursor("Допуска КП.Допуска крановщики",
