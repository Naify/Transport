...
AGroupAdmissionModel::AGroupAdmissionModel(int vcId, QObject *parent) :
...
AGroupAdmissionModel::~AGroupAdmissionModel()
{
}

void AGroupAdmissionModel::init()
{
    load();
}

void AGroupAdmissionModel::load()
{
...
void AGroupAdmissionModel::makeRow(const ODS::IObject &obj)
{
    ...
    datetime.setTime_t(obj.getDateTimeAttr("Дата аттестации").getDateTime());
    item->setText(datetime.toString("dd.MM.yyyy"));
    list<<item;

    item = new QStandardItem;
    item->setData(Result, TypeRole);
    if (obj.getBoolAttr("Результат аттестации")){
        item->setText("Допущен");
    } else {
        item->setText("Не допущен");
    }
    list<<item;
...
void AGroupAdmissionModel::addEmptyRow()
{
    QList<QStandardItem*> list;
...
void AGroupAdmissionModel::choosePerson()
{
...
void AGroupAdmissionModel::remove(const QModelIndex &index)
{
...
bool AGroupAdmissionModel::save()
{
    ...
    for (int i = 0; i< rowCount(); i++){
        ...
        if (id > 0)
            ...
            obj = ODS::OdsInterface::self()->iobjectManager().createIObject("Допуска КП.Допуска группы А");
...
bool AGroupAdmissionModel::saveItem(QStandardItem *item, ODS::IObject &obj)
{
    ...
    while(item)
    {
        ...
        case Date2:{
            ...
            obj.setAttr("Дата аттестации", cDate);
            break;
        }
        case Result:
        {
            if (item->data(AttestResultRole).toInt() == 1){
                obj.setAttr("Результат аттестации", true);
            } else {
                obj.setAttr("Результат аттестации", false);
            }
            break;
        }
        case NumAndDate:{
...
void AGroupAdmissionModel::changeVc(int vcId)
{
    removeRows(0, rowCount());
    currentVcid = vcId;

    ODS::IObjectCursor cursor = ODS::OdsInterface::self()->iobjectManager().getIObjectCursor("Допуска КП.Допуска группы А",
