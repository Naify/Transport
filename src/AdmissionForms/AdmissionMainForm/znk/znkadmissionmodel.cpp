...
ZnkAdmissionModel::ZnkAdmissionModel(int vcId, QObject *parent):
...
ZnkAdmissionModel::~ZnkAdmissionModel()
{
}

void ZnkAdmissionModel::init()
{
    load();
}

void ZnkAdmissionModel::load()
{
    ...
    ODS::IObjectCursor cursor = ODS::OdsInterface::self()->iobjectManager().getIObjectCursor("Допуска КП.Допуска ЗНК",filter);
...
void ZnkAdmissionModel::makeRow(const ODS::IObject &obj)
{
...
void ZnkAdmissionModel::addEmptyRow()
{
    QList<QStandardItem*> list;
...
void ZnkAdmissionModel::choosePerson()
{
...
void ZnkAdmissionModel::remove(const QModelIndex &index)
{
...
bool ZnkAdmissionModel::save()
{
    ...
    for (int i = 0; i< rowCount(); i++){
        ...
        if (id > 0)
            ...
            obj = ODS::OdsInterface::self()->iobjectManager().createIObject("Допуска КП.Допуска ЗНК");
...
bool ZnkAdmissionModel::saveItem(QStandardItem *item, ODS::IObject &obj)
{
...
void ZnkAdmissionModel::changeVc(int vcId)
{
    removeRows(0, rowCount());
    currentVcid = vcId;

    ODS::IObjectCursor cursor = ODS::OdsInterface::self()->iobjectManager().getIObjectCursor("Допуска КП.Допуска ЗНК",
