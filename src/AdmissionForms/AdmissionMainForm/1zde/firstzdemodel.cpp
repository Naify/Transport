...
FirstZdeModel::FirstZdeModel(int vcId, QObject *parent) :
...
FirstZdeModel::~FirstZdeModel()
{
}

void FirstZdeModel::init()
{
    load();
}

void FirstZdeModel::load()
{
    ...
    ODS::IObjectCursor cursor = ODS::OdsInterface::self()->iobjectManager().getIObjectCursor("Допуска КП.Допуска 1ЗНЖДЭ", filter);
...
void FirstZdeModel::makeRow(const ODS::IObject &obj)
{
...
void FirstZdeModel::addEmptyRow()
{
    QList<QStandardItem*> list;
...
void FirstZdeModel::choosePerson()
{
...
void FirstZdeModel::remove(const QModelIndex &index)
{
...
bool FirstZdeModel::save()
{
    ...
    for (int i = 0; i< rowCount(); i++){
        ...
        if (id > 0)
            ...
            obj = ODS::OdsInterface::self()->iobjectManager().createIObject("Допуска КП.Допуска 1ЗНЖДЭ");
...
bool FirstZdeModel::saveItem(QStandardItem *item, ODS::IObject &obj)
{
...
void FirstZdeModel::changeVc(int vcId)
{
    removeRows(0, rowCount());
    currentVcid = vcId;

    ODS::IObjectCursor cursor = ODS::OdsInterface::self()->iobjectManager().getIObjectCursor("Допуска КП.Допуска 1ЗНЖДЭ",
