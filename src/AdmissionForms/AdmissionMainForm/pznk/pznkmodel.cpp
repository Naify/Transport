...
PznkModel::PznkModel(int vcId, QObject *parent) :
...
void PznkModel::load()
{
    ...
    ODS::IObjectCursor cursor = ODS::OdsInterface::self()->iobjectManager().getIObjectCursor("Допуска КП.Допуска помощники начальника караула",filter);
...
void PznkModel::makeRow(const ODS::IObject &obj)
{
...
void PznkModel::remove(const QModelIndex &index)
{
...
bool PznkModel::save()
{
    ...
    for (int i = 0; i< rowCount(); i++){
        ...
        if (id > 0)
            ...
            obj = ODS::OdsInterface::self()->iobjectManager().createIObject("Допуска КП.Допуска помощники начальника караула");
...
bool PznkModel::saveItem(QStandardItem *item, ODS::IObject &obj)
{
...
void PznkModel::changeVc(int vcId)
{
    removeRows(0, rowCount());
    currentVcid = vcId;

    ODS::IObjectCursor cursor = ODS::OdsInterface::self()->iobjectManager().getIObjectCursor("Допуска КП.Допуска помощники начальника караула",
