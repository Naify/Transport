...
AkAdmissionModel::AkAdmissionModel(int vcId, QObject *parent) :
...
AkAdmissionModel::~AkAdmissionModel()
{
}

void AkAdmissionModel::init()
{
    load();
}

void AkAdmissionModel::load()
{
    ...
    ODS::IObjectCursor cursor = ODS::OdsInterface::self()->iobjectManager().getIObjectCursor("Допуска КП.Допуска ЗНАК",filter);
...
void AkAdmissionModel::makeRow(const ODS::IObject &obj)
{
...
void AkAdmissionModel::addEmptyRow()
{
    QList<QStandardItem*> list;
...
void AkAdmissionModel::choosePerson()
{
...
void AkAdmissionModel::remove(const QModelIndex &index)
{
...
bool AkAdmissionModel::save()
{
    ...
    for (int i = 0; i< rowCount(); i++){
        ...
        if (id > 0)
            ...
            obj = ODS::OdsInterface::self()->iobjectManager().createIObject("Допуска КП.Допуска ЗНАК");
...
bool AkAdmissionModel::saveItem(QStandardItem *item, ODS::IObject &obj)
{
...
void AkAdmissionModel::changeVc(int vcId)
{
    removeRows(0, rowCount());
    currentVcid = vcId;

    ODS::IObjectCursor cursor = ODS::OdsInterface::self()->iobjectManager().getIObjectCursor("Допуска КП.Допуска ЗНАК",
