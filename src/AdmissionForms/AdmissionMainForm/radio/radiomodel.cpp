...
RadioModel::RadioModel(int vcId, QObject *parent) :
...
void RadioModel::load()
{
    ...
    ODS::IObjectCursor cursor = ODS::OdsInterface::self()->iobjectManager().getIObjectCursor("Допуска КП.Допуска расчет радиостанции",filter);
...
void RadioModel::makeRow(const ODS::IObject &obj)
{
...
void RadioModel::remove(const QModelIndex &index)
{
...
bool RadioModel::save()
{
    ...
    for (int i = 0; i< rowCount(); i++){
        ...
        if (id > 0)
            ...
            obj = ODS::OdsInterface::self()->iobjectManager().createIObject("Допуска КП.Допуска расчет радиостанции");
...
bool RadioModel::saveItem(QStandardItem *item, ODS::IObject &obj)
{
...
void RadioModel::changeVc(int vcId)
{
    removeRows(0, rowCount());
    currentVcid = vcId;

    ODS::IObjectCursor cursor = ODS::OdsInterface::self()->iobjectManager().getIObjectCursor("Допуска КП.Допуска расчет радиостанции",
