...
RazvodModel::RazvodModel(int vcId, QObject *parent) :
...
void RazvodModel::init()
{
    load();
}

void RazvodModel::load()
{
    ...
    ODS::IObjectCursor cursor = ODS::OdsInterface::self()->iobjectManager().getIObjectCursor("Допуска КП.Допуска разводящие",filter);
...
void RazvodModel::makeRow(const ODS::IObject &obj)
{
...
void RazvodModel::remove(const QModelIndex &index)
{
...
bool RazvodModel::save()
{
    ...
    for (int i = 0; i< rowCount(); i++){
        ...
        if (id > 0)
            ...
            obj = ODS::OdsInterface::self()->iobjectManager().createIObject("Допуска КП.Допуска разводящие");
...
bool RazvodModel::saveItem(QStandardItem *item, ODS::IObject &obj)
{
...
void RazvodModel::changeVc(int vcId)
{
    removeRows(0, rowCount());
    currentVcid = vcId;

    ODS::IObjectCursor cursor = ODS::OdsInterface::self()->iobjectManager().getIObjectCursor("Допуска КП.Допуска разводящие",
