...
DamagedSiAdmissionModel::DamagedSiAdmissionModel(int vcId, QObject *parent) :
...
DamagedSiAdmissionModel::~DamagedSiAdmissionModel()
{
}

void DamagedSiAdmissionModel::init()
{
    load();
}

void DamagedSiAdmissionModel::load()
{
    ...
    ODS::IObjectCursor cursor = ODS::OdsInterface::self()->iobjectManager().getIObjectCursor("Допуска КП.Допуска к аварийным СИ",filter);
...
void DamagedSiAdmissionModel::makeRow(const ODS::IObject &obj)
{
    ...
    item = new QStandardItem;
    item->setData(Si, TypeRole);
    catalog = catalMgr.getCatalogByName("Классификатор грузов");
    if (catalog){
        if (obj.getIntAttr("СИ") != 0){
            IRS::CatalogEntry::ptr entry = catalog->entryById(obj.getIntAttr("СИ"));
            item->setText(entry->stringValue("Наименование"));
        }
    } else {
        QMessageBox::critical(0,"Ошибка", "Недействительный 'Классификатор грузов'");
    }
    list<<item;
    ...
    item = new QStandardItem;
    item->setData(Place, TypeRole);
    catalog = catalMgr.getCatalogByName("Классификатор воинских частей");
    if (catalog){
        if (obj.getIntAttr("Место сдачи") != 0){
            IRS::CatalogEntry::ptr entry = catalog->entryById(obj.getIntAttr("Место сдачи"));
...
void DamagedSiAdmissionModel::addEmptyRow()
{
    QList<QStandardItem*> list;
...
void DamagedSiAdmissionModel::choosePerson()
{
...
void DamagedSiAdmissionModel::remove(const QModelIndex &index)
{
...
bool DamagedSiAdmissionModel::save()
{
    ...
    for (int i = 0; i< rowCount(); i++){
        ...
        if (id > 0)
            ...
            obj = ODS::OdsInterface::self()->iobjectManager().createIObject("Допуска КП.Допуска к аварийным СИ");
...
bool DamagedSiAdmissionModel::saveItem(QStandardItem *item, ODS::IObject &obj)
{
    ...
    while(item)
    {
        ...
        case Date:
            break;
        case Si:
            obj.setAttr("СИ", item->data(SiIdRole).toInt());
            break;
        case NumAndDate:
            obj.setAttr("Номер приказа", item->text());
            break;
        case Place:
            obj.setAttr("Место сдачи", item->data(PlaceRole).toInt());
            break;
        }
...
void DamagedSiAdmissionModel::changeVc(int vcId)
{
    removeRows(0, rowCount());
    currentVcid = vcId;

    ODS::IObjectCursor cursor = ODS::OdsInterface::self()->iobjectManager().getIObjectCursor("Допуска КП.Допуска к аварийным СИ",
